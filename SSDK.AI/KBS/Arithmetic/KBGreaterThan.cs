...
namespace SSDK.AI.KBS.Arithmetic
{
    ...
    /// </summary>
    public sealed class KBGreaterThan : KBFactor
    {
        /// <summary>
        ...
        public KBGreaterThan(KBFactor lhs, KBFactor rhs)
        {
            LHS = lhs;
            RHS = rhs;
        }
            ...
            if (!Solved)
            {
                ...
                if (type == KBSolveType.NoSolution || type == KBSolveType.Other)
                {
                    if (LHS.Solved && RHS.Solved)
                    {
                        KBFactor left = LHS.Calculate();
                        KBFactor right = RHS.Calculate();
                        type = left.Apply('>', right).IsBooleanTrue ? KBSolveType.SolveTrue : KBSolveType.SolveFalse;
                    }
                }
