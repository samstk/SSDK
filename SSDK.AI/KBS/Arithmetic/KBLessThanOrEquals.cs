...
namespace SSDK.AI.KBS.Arithmetic
{
    ...
    /// </summary>
    public sealed class KBLessThanOrEquals : KBFactor
    {
        /// <summary>
        ...
        public KBLessThanOrEquals(KBFactor lhs, KBFactor rhs)
        {
            LHS = lhs;
            RHS = rhs;
        }
        ...
        {
            return LHS.Calculate().Apply('L', RHS.Calculate());
        }
        ...
        public override KBFactor HasConflict()
        {
            ...
            return Solved && Assertion && left as object != null && right as object  != null && left.Apply('L', right).IsBooleanFalse ? this : null;
        }
            ...
            if (!Solved)
            {
                ...
                if (type == KBSolveType.NoSolution || type == KBSolveType.Other)
                {
                    if (LHS.Solved && RHS.Solved)
                    {
                        KBFactor left = LHS.Calculate();
                        KBFactor right = RHS.Calculate();
                        type = left.Apply('L', right).IsBooleanTrue ? KBSolveType.SolveTrue : KBSolveType.SolveFalse;
                    }
                }
